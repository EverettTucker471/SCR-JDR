...
def DataLoader(name, normalize_data):
    ...
    # Custom dataloader for ogbn-products
    if name in ["ogbn-products"]:
        root_path = '../'
        path = osp.join(root_path, 'data', name)

        # Load OGB dataset
        dataset = NodePropPredDataset(name=name, root=path)

        # Convert to PyG Data object
        data = from_ogb_node_proppred(dataset)[0]

        # Apply optional NormalizeFeatures() to match Planetoid/Amazon logic
        if normalize_data:
